...
INI: XML_DECLARATION ROOT EOF               {/*$1[0].printTest(0);console.log($1[0].getTree());*/
    ...
    | ROOT EOF                              {
                                            ...
                                            errors.push({ tipo: "Sintáctico", error: "Falta declaración del XML", origen: "XML", linea: @1.first_line, columna: @1.first_column+1 });
...
ATTRIBUTE : tk_attribute_name tk_string     {attr = new Atributo($1.slice(0, -1), $2.slice(1,-1), this._$.first_line, this._$.first_column+1);
            ...
            | cadena_err tk_string          { $$ = null;
                                            errors.push({ tipo: "Lexico", error: "Nombre del atributo no puede empezar con dígitos.", origen: "XML", linea: @1.first_line, columna: @1.first_column+1 });
            ...
            | cadena_err                    { $$ = null;
                                            errors.push({ tipo: "Lexico", error: "Nombre del atributo no puede empezar con dígitos, y debe tener signo = y atributo a continuación.", origen: "XML", linea: @1.first_line, columna: @1.first_column+1 });
...
CHILDREN: CHILDREN XML                                      {if($1 != null && $2 != null){ $1.push($2); $$ = $1; } else{$$ = null;}
                                                            prod_1 = grammar_stack.pop();
                                                            prod_2 = grammar_stack.pop();
                                                             grammar_stack.push({'CHILDREN -> CHILDREN XML {﹩1.push(﹩2); ﹩﹩ = $1;}':[prod_2,  prod_1]});
                                                            }
